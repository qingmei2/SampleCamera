...
internal class SizeMap {
     ...
     */
    fun remove(ratio: AspectRatio)= mRatios.remove(ratio)

    fun ratios(): Set<AspectRatio> = mRatios.keys

    fun sizes(ratio: AspectRatio): SortedSet<Size>? = mRatios[ratio]

    fun clear() = mRatios.clear()
}